...
class DnifClient:
    ...
    def _login(self, email, password):
        """
        DNIF login is a TWO-STEP handshake. Confirmed two ways now:
        first from a HAR capture of the browser flow, and second --
        definitively -- from DNIF's own frontend source (index.js,
        the Login component's onSubmit handler), which settles the
        cstPass formula exactly rather than leaving it inferred:

          1. POST /lc/users/init   {"emailId": ...}
             Validates the email exists BEFORE any password is
             involved, and returns the fields the real login logic
             branches on:
               - Invalid/unknown email -> 400,
                 {"message": "Email ID or Password is incorrect", ...}
               - Valid email -> 200,
                 {"data": {"ad_auth": bool, "ad_configured": bool,
                            "chId": "...", "mfa_flag": bool}, ...}

          2. POST /lc/users/login  {"emailId": ..., "cstPass": ...}
             cstPass construction, taken directly from source:

               data.ad_auth && data.ad_configured
                 ? btoa(password)                    // AD/SSO accounts
                 : md5(chId + md5(password))          // local accounts

             For local accounts (the common case, and the only one
             confirmed against real captured logins): MD5 the
             password first, then concatenate that hex digest AFTER
             chId (no separator), then MD5 again.

             For AD/SSO-configured accounts: cstPass is just
             base64(password) -- NOT hashed at all, only base64
             encoded. That's effectively reversible plaintext,
             protected solely by TLS in transit. Implemented here
             because it's exactly what DNIF's own frontend does, not
             a design choice made by this client -- worth flagging to
             whoever owns DNIF's auth design if not already known,
             since it's meaningfully weaker than the local-account
             path.

        MFA is the one branch this client still can't complete
        end-to-end: mfa_flag=true means a verification step exists
        beyond what's captured here (not present in the Login
        component itself -- likely a separate component/route this
        client doesn't have visibility into). Raises clearly rather
        than pretending to handle it.
        """
        init_resp = requests.post(
            f"https://{self.host}/lc/users/init",
        ...
        if init_resp.status_code != 200:
            try:
                detail = init_resp.json()
            except ValueError:
                detail = init_resp.text
            raise RuntimeError(
                f"DNIF rejected the email itself (POST /lc/users/init returned "
                f"{init_resp.status_code} for emailId={email!r}): {detail}\n"
                f"This means the email doesn't exist as an account on this specific "
                f"DNIF instance ({self.host}) -- not a password problem. Confirm this "
                f"is the right account for this host, not one that only exists on a "
                f"different DNIF instance/tenant."
            )

        init_data = init_resp.json().get("data", {})
        chid = init_data.get("chId")
        if init_data.get("mfa_flag"):
            raise RuntimeError(
                f"emailId={email!r} has MFA enabled on {self.host} -- this client doesn't "
                f"implement the MFA verification step (not present in the login source "
                f"consulted). Use an account with MFA disabled for automation, or tell me "
                f"if this needs adding and share the MFA verification component's source "
                f"the same way you shared the login source."
            )
        if not chid:
            raise RuntimeError(f"/lc/users/init succeeded but returned no chId: {init_data}")

        if init_data.get("ad_auth") and init_data.get("ad_configured"):
            cst_pass = base64.b64encode(password.encode()).decode()
        else:
            cst_pass = hashlib.md5((chid + hashlib.md5(password.encode()).hexdigest()).encode()).hexdigest()
        ...
        if resp.status_code != 200:
            ...
            raise RuntimeError(
                ...
                f"Email was already confirmed valid by /lc/users/init, and cstPass now uses "
                f"the confirmed real formula (md5(chId+md5(password)), from DNIF's own "
                f"frontend source) -- so at this point a rejection most likely means the "
                f"password value itself is wrong. Re-check the credentials file directly."
            )
        body = resp.json()
