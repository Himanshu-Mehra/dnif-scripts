...
class DnifClient:
    ...
    def _login(self, email, password):
        """
        DNIF login is a TWO-STEP handshake, confirmed from a HAR
        capture of the actual browser flow -- my first pass at this
        client missed the first step entirely (I'd filtered the
        earlier HAR for "login"/"auth"/"signin" in the URL, which
        doesn't match "/lc/users/init").

          1. POST /lc/users/init   {"emailId": ...}
             Validates the email exists BEFORE any password is
             involved. Also returns whether this account uses AD/SSO
             auth (ad_auth) and whether MFA is required (mfa_flag) --
             this client only handles the plain local-account,
             no-MFA case; see the checks below for what happens
             otherwise.
             - Invalid/unknown email -> 400,
               {"message": "Email ID or Password is incorrect", ...}
             - Valid email -> 200,
               {"data": {"ad_auth": false, "ad_configured": false,
                          "chId": "...", "mfa_flag": false}, ...}

          2. POST /lc/users/login  {"emailId": ..., "cstPass": <md5>}
             Only called if step 1 succeeded. This is the step that
             actually returns the SSID.

        No cookies are set on the /init response (checked directly in
        the HAR) -- the two calls aren't linked by session state, so
        this can't be why calling /login alone previously failed for
        a WRONG email; that was a plain "this email doesn't exist on
        this instance" rejection. But doing the real two-step flow
        still matters: it fails fast with a clean, unambiguous error
        (from a call that only takes an email, no password/hash logic
        involved yet) instead of only finding out at the password
        step.
        """
        ...
        if init_data.get("ad_auth"):
            raise RuntimeError(
                f"emailId={email!r} is an AD/SSO-authenticated account (ad_auth=true) on "
                f"{self.host} -- this client only implements the local-account cstPass "
                f"login flow. An AD-backed account needs a different auth flow this "
                f"client doesn't support yet; tell me if this needs adding."
            )
        if init_data.get("mfa_flag"):
            raise RuntimeError(
                f"emailId={email!r} has MFA enabled on {self.host} -- this client only "
                f"implements the no-MFA login flow (mfa_flag=false in your captured HAR). "
                f"An MFA-enabled account needs an extra verification step this client "
                f"doesn't support yet; tell me if this needs adding, or use an account "
                f"with MFA disabled for automation."
            )

        cst_pass = hashlib.md5(password.encode()).hexdigest()  # confirmed correct against a real login
