...
case "${os}" in
        ubuntu|centos)
                ...
                do
                        ...
                        if [ -e /DNIF/EB/docker-compose.yaml ]; then
                                dc_container_name=$(cat /DNIF/EB/docker-compose.yaml | grep "$container_name" | awk '{print $2}')
                                if [ "$dc_container_name" == "$container_name" ]; then
                                        dcfound=$dc_container_name
                                        dcversion=$(cat /DNIF/EB/docker-compose.yaml | grep 'image' | grep 'eventbus' | awk '{print $2}' | cut -d':' -f2 | tr -d '"')
                                fi
                        fi
                        if [ "$dcfound" == "$container_name" ]; then
                                ...
                                if (( $int_tag == $int_dcversion )); then
                                        ...
                                        if [ "$container_name" == "core-v9" ]; then
                                                ...
                                                echo -e "[-] Updating eventbus docker-compose..\n"
                                                file="/DNIF/EB/docker-compose.yaml"
                                                ...
                                                for path in $(cat "$mad_file"); do
                                                        echo -e "\n[-] Updating micro adapter $path/docker-compose...\n"
                                                        mad_dcversion=$(cat $path/docker-compose.yaml | grep 'image' | grep 'adapter' | awk '{print $2}' | cut -d':' -f2)
                                                        ...
                                                        if (( $int_tag == $int_dcversion )); then
                                                                ...
                                                                echo -e "\n[-] Updating micro adapter $path/docker-compose...\n"
                                                                sed -i s/"$mad_dcversion"/"$tag"/g $path/docker-compose.yaml
                                                                echo -e "\n[-] Starting docker container\n"
                                                                cd $path
                                                                docker-compose up -d
                                                                docker ps -a
